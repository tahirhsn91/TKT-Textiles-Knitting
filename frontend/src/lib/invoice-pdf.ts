...
export async function downloadInvoicePdf(inv: InvoiceDetail): Promise<void> {
  // jsPDF (~390 kB min) is only needed when the user actually downloads the
  // PDF — pull it in on demand so the invoicing screen doesn't pay the cost
  // at page load (the page-level code split already covers recharts).
...
// Number-to-words formatting now lives in lib/invoice-amount.ts (shared with
// the invoicing screen, which renders the amount-in-words row without loading
// jsPDF).
