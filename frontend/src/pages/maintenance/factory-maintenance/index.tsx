...
import { Plus, Pencil, RotateCcw, Factory, Eye, Trash2 } from "lucide-react";
import { format } from "date-fns";
import { useQueryClient } from "@tanstack/react-query";

import {
  useGetFactoryMaintenanceList,
  useSetFactoryMaintenanceStatus,
  type FactoryMaintenanceRow,
} from "@/hooks/use-factory-maintenance";
import { FactoryMaintenanceDialog } from "./add-factory-maintenance-dialog";
import { FactoryMaintenanceAnalytics } from "./analytics-tab";
...
export default function FactoryMaintenanceList() {
  ...
  const [pendingCancel, setPendingCancel] = useState<FactoryMaintenanceRow | null>(null);

  const { data, isLoading } = useGetFactoryMaintenanceList(date, status, page);
  const setStatusMut = useSetFactoryMaintenanceStatus();
  ...
  const doCancel = (r: FactoryMaintenanceRow) => {
    setStatusMut.mutate(
  ...
  const doRestore = (r: FactoryMaintenanceRow) => {
    setStatusMut.mutate(
        ...
        <header className="flex items-start justify-between gap-4 border-b pb-5">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Factory maintenance history for <span className="num">{dateLabel}</span>.
            </p>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  <TableHead className="eyebrow h-11 px-5">Category</TableHead>
                ...
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={COLUMN_COUNT} className="py-10 text-center text-muted-foreground">
                      No {status === "submitted" ? "active" : "cancelled"} factory maintenance for {dateLabel} yet.
                  ...
                  rows.map((r) => {
                    ...
                    <TableRow
                      ...
                      onClick={isMobile ? () => (cancelled ? openView(r.id) : openEdit(r.id)) : undefined}
                    >
                      ...
                      <TableCell className="max-w-[20rem] whitespace-pre-wrap text-sm">{r.maintenanceWork}</TableCell>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingCancel && (
                <>
                  <span className="font-medium text-foreground">{pendingCancel.category}</span> —{" "}
